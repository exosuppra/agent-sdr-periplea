...
- Sourcing (Apify) : la recherche Google (`site:linkedin.com/in`) tourne sur le plan gratuit, environ 0,001 à 0,002 $ par page (10 profils). L'enrichissement de profil (acteur harvestapi : entreprise, poste, email pro vérifié) suppose le plan payant Apify (Starter, 29 $/mois), le plan gratuit étant plafonné à 10 exécutions de cet acteur. Le coût d'usage de l'enrichissement reste d'environ 0,01 $ par prospect, soit 4 à 5 $ pour 400 prospects par mois, couverts par les crédits inclus dans l'abonnement. Cet abonnement n'est d'ailleurs pas dédié à cet agent : Apify est une plateforme d'infrastructure mutualisable, réutilisable pour de nombreux autres scrapers, automatisations et agents IA, donc son coût ne s'impute pas entièrement à ce seul cas d'usage.
